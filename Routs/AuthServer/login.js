...
router.post('/', async (req, res) => {
    ...
    const accessToken = JWT.sign({ username, password }, process.env.ACCESS_SECRET_TOKEN, { expiresIn: '2m' })
    ...
    const refreshToken = JWT.sign({ username, password }, process.env.REFRESH_SECRET_TOKEN)
